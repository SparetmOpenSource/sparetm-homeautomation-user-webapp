...
const Scheduler: React.FC<SchedulerProps> = ({ darkTheme, deviceId }) => {
    ...
    const { refetch, isFetching } = useGetDeviceSchedules(
        deviceId,
        admin,
        profile,
        darkTheme,
        (data: any) => {
            if (data && data.data && data.data.data) {
                const sorted = data.data.data.sort((a: any, b: any) => a.time.localeCompare(b.time));
                setSchedules(sorted);
            }
        }
    );
    ...
    const { mutate: addScheduleMutation } = useAddDeviceSchedule(
        admin,
        profile,
        darkTheme,
        () => {
    ...
    const { mutate: deleteScheduleMutation } = useDeleteDeviceSchedule(
        darkTheme,
        () => {
