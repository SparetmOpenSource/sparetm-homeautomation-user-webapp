...
import AudioProgressBar from '../../../../CommonComponents/Slide/AudioProgressBar/AudioProgressBar';
import {
    catchError,
    invalidateQueries,
    trimToNChars,
...
const SpotifyCurrentPlayback = ({
    ...
    const playbackType = data?.data?.body?.currently_playing_type;
    ...
    const renderPlaybackIcon = () => {
        ...
        return (
            <img
                ...
                src={data?.data?.body?.item?.album?.images[0]?.url}
                alt="song_image"
                loading="lazy"
                width="95%"
                height="95%"
    ...
    const { mutate: next } = usePostUpdateData(
        `${featureUrl.spotify_base_url}?data=next&id=${data?.data?.body?.device?.id}`,
        updateHeaderConfig,
        onNavSuccess,
        onNavError,
    );
    ...
    const { mutate: previous } = usePostUpdateData(
        `${featureUrl.spotify_base_url}?data=previous&id=${data?.data?.body?.device?.id}`,
        updateHeaderConfig,
        onNavSuccess,
        onNavError,
    );
    ...
    const togglePlayPause = () => {
        ...
        if (!isPlaying) {
            playMutation.mutate({
                device_ids: [`${deviceId}`],
                context_uri: `${data?.data?.body?.context?.uri}`,
                offset: {
                    uri: `${data?.data?.body?.item?.uri}`,
                },
                position_ms: `${data?.data?.body?.progress_ms}`,
            });
        } else {
    ...
    useEffect(() => {
        data?.data?.body?.is_playing === true
            ? setIsPlaying(true)
            : setIsPlaying(false);
        if (data?.data?.body?.progress_ms) {
            setProgress(data.data.body.progress_ms);
        }
    }, [data]);
                    ...
                    >
                        ...
                        >
                            <h1 style={{ color: color?.text }}>
                                {trimToNChars(data?.data?.body?.item?.name, 15)}
                            </h1>
                            <p style={{ color: color?.icon_font }}>
                                {trimToNChars(
                                    data?.data?.body?.item?.album?.artists[0]?.name,
                                    12,
                                )}
                            </p>
                            {windowSize === 'S' && (
                                ...
                                >
                                    ...
                                    >
                                        {' '}
                                        {trimToNChars(
                                            data?.data?.body?.item?.album?.name,
                                            12,
                                        )}
                                    </span>
                                </p>
                        ...
                        {(windowSize === 'S' || windowSize === 'XL') && (
                            ...
                            >
                                <AudioProgressBar
                                    totalTimeMs={data?.data?.body?.item?.duration_ms}
